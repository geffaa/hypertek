...
import { Swords, Gamepad2, Clock } from "lucide-react";
...
// ── Static preview data (matches Excel format) ────────────────────────────────
const PREVIEW_QUESTS = [
  { _id: "q-1098474", status: "accepted",  title: "100usdc / 1M Oil",      offering: "100 USDC",   reward: 6,   pickupPlanet: "Utfulyjhv",  dropOffPlanet: "Wreerk",   acceptedByWallet: "0xFireFly0000",     createdAt: new Date(Date.now() - 14295000).toISOString() },
  { _id: "q-1098469", status: "accepted",  title: "1000usdc / 5M Food/Oil", offering: "1000 USDC",  reward: 50,  pickupPlanet: "Panhd",      dropOffPlanet: "Ghgbvr",   acceptedByWallet: "0xBumbleBee0000",   createdAt: new Date(Date.now() - 42481000).toISOString() },
  { _id: "q-1098453", status: "accepted",  title: "750usdc / 2M Wood/Food", offering: "750 USDC",   reward: 37,  pickupPlanet: "Rbthfg5h65", dropOffPlanet: "Nbrtdv",   acceptedByWallet: "0xKissMyA0000",    createdAt: new Date(Date.now() - 48840000).toISOString() },
  { _id: "q-1098437", status: "accepted",  title: "120usdc / 1M Food/Ore",  offering: "120 USDC",   reward: 7,   pickupPlanet: "Z234",       dropOffPlanet: "Bftyuhgdb", acceptedByWallet: "0xYoMomma0000",    createdAt: new Date(Date.now() - 7792000).toISOString() },
  { _id: "q-1098436", status: "open",      title: "100usdc / 1M Ore",       offering: "100 USDC",   reward: 6,   pickupPlanet: "V56h6k6",    dropOffPlanet: "Jjnbfdb",  acceptedByWallet: null,               createdAt: new Date(Date.now() - 3703000).toISOString() },
  { _id: "q-1098420", status: "open",      title: "500usdc / 3M Goods",     offering: "500 USDC",   reward: 25,  pickupPlanet: "Xtra7",      dropOffPlanet: "Orbion",   acceptedByWallet: null,               createdAt: new Date(Date.now() - 1200000).toISOString() },
];
  ...
  const fetchQuests = useCallback(async () => {
    ...
    try {
      ...
      if (fetched.length > 0) {
        ...
        setQuests(PREVIEW_QUESTS.filter((q) => q.status === statusFilter || statusFilter === "all"));
    ...
    <div className="py-6 relative overflow-hidden">
      ...
      {loading ? (
        ...
        <div className="rounded-2xl overflow-x-auto"
          ...
          {/* Table header */}
          <div
            className="grid min-w-[720px] px-4 py-3 text-[10px] font-semibold uppercase tracking-widest text-white/30"
            style={{
              background: "rgba(10,20,60,0.6)",
              gridTemplateColumns: "1fr 1.2fr 1.2fr 1.8fr 1.2fr 1fr 1.2fr",
            }}
          >
          ...
          {quests.map((q, i) => {
            ...
            return (
              <div
                key={q._id}
                className="grid min-w-[720px] px-4 py-3 items-center"
                style={{
              ...
              >
                ...
                <div className="flex flex-col gap-0.5">
                  ...
                  <span
                    ...
                    style={{ background: colors.bg, border: `1px solid ${colors.border}`, borderRadius: 3, padding: "1px 4px", display: "inline-block", width: "fit-content" }}
                  >
                    {q.status}
                  </span>
                </div>
                ...
                <span className="text-white/50 text-xs">
                  {q.pickupPlanet || <span className="text-white/20 italic">—</span>}
                </span>

                {/* Drop Off Planet */}
                <span className="text-white/50 text-xs">
                  {q.dropOffPlanet || <span className="text-white/20 italic">—</span>}
                </span>

...
function DesktopCard({ game, isActive, onEnter, onLeave, onClick, index, t }) {
  ...
  return (
    ...
    >
      ...
      {/* Dark overlay */}
      <div
        className="absolute inset-0 transition-all duration-500 z-[2]"
        style={{
          background: isActive
...
export default function Preview() {
  ...
  return (
    <div className="relative text-white min-h-screen flex flex-col" style={{ background: "#060614" }}>
      ...
      <header className="relative z-20 w-full" style={{ background: "rgba(0, 17, 66, 0.55)", backdropFilter: "blur(20px)", borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
        <div className="w-full max-w-[1480px] mx-auto px-6 md:px-14 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div className="flex flex-col">
              <span className="text-white font-bold text-lg md:text-xl tracking-[0.2em] uppercase leading-tight" style={{ fontFamily: "Orbitron, sans-serif" }}>HYPER TEK</span>
              <span className="text-white/40 text-[9px] md:text-[10px] tracking-[0.3em] uppercase">Building Worlds, One Game At A Time</span>
            </div>
          </div>
        </div>
      </header>
      ...
      <main className="relative z-10 flex-1 w-full max-w-[1480px] mx-auto px-6 md:px-14 xl:px-18 2xl:px-20">
        {/* Hero text */}
        <motion.div className="pt-16 md:pt-24 pb-10 md:pb-14 text-center" initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, ease: [0.22, 1, 0.36, 1] }}>
          <span className="text-[10px] md:text-[11px] tracking-[0.4em] uppercase font-bold text-blue-400 block mb-4" style={{ fontFamily: "Orbitron, sans-serif" }}>Explore Our Universe</span>
          <h1 className="font-goldman uppercase text-3xl md:text-5xl xl:text-[56px] leading-tight mb-5" style={{ textShadow: "0 0 80px rgba(0,42,168,0.4)" }}>Choose Your Path</h1>
          <p className="text-white/50 text-[15px] md:text-[17px] max-w-[600px] mx-auto leading-relaxed">Three interconnected games, one shared universe. Click to discover the world that calls to you.</p>
        </motion.div>

...
const createItem = async (req, res) => {
  try {
    ...
    const item = await LandSchema.create({
      title,
      serialNumber,
      price,
      userId,
...
const getItems = async (req, res) => {
  try {
    const items = await LandSchema.find().sort({ createdAt: -1 });
...
const getItemById = async (req, res) => {
  try {
    const item = await LandSchema.findById(req.params.id);
...
const updateItem = async (req, res) => {
  try {
    ...
    const updatedItem = await LandSchema.findByIdAndUpdate(
      req.params.id,
...
const deleteItem = async (req, res) => {
  try {
    const deletedItem = await LandSchema.findByIdAndDelete(req.params.id);
    if (!deletedItem)

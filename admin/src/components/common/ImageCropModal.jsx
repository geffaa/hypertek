...
function getCroppedBlob(image, canvas, crop, fileName) {
  ...
  canvas.width  = Math.floor(crop.width  * scaleX * dpr);
  canvas.height = Math.floor(crop.height * scaleY * dpr);
...
export default function ImageCropModal({ src, fileName, onConfirm, onCancel, aspect: forcedAspect }) {
  ...
  const locked = forcedAspect != null;

  const [crop,          setCrop]          = useState();
  const [completedCrop, setCompletedCrop] = useState(null);
  const [aspect,        setAspect]        = useState(locked ? forcedAspect : 1);
  ...
  return (
    ...
    >
      <div
        ...
        style={{ background: "#0f1117", width: "min(560px, 95vw)", maxHeight: "90vh", gap: 10 }}
      >
        {/* Header */}
        ...
        {/* Aspect ratio tabs — hidden when ratio is locked externally */}
        {locked && (
          <div className="flex items-center gap-2 flex-shrink-0">
            <span className="text-white/30 text-xs">Ratio locked:</span>
            <span className="px-2.5 py-0.5 rounded text-xs font-semibold" style={{ background: "rgba(0,42,168,0.25)", border: "1px solid rgba(0,80,255,0.4)", color: "#60a5fa" }}>
              {forcedAspect === 16 / 9 ? "16:9" : forcedAspect === 4 / 3 ? "4:3" : `${forcedAspect.toFixed(2)}:1`}
            </span>
          </div>
        )}
        {!locked && (
        ...
        <div
          ...
          style={{ height: "calc(90vh - 180px)", minHeight: 120 }}
        >
          <ReactCrop
            crop={crop}
